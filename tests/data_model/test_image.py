...
from caikit_computer_vision.data_model import Image as CaikitImage
...
def test_valid_image_initializations(happy_kwargs):
    ...
    im = CaikitImage(**happy_kwargs)
    assert isinstance(im, CaikitImage)
...
def test_as_pil():
    ...
    pimg = CaikitImage(PIL_IMG_DATA).as_pil()
...
def test_as_numpy():
    ...
    dimg = CaikitImage(PIL_IMG_DATA)
    expected_shape = (dimg.rows, dimg.columns, dimg.channels)
    npimg = CaikitImage(PIL_IMG_DATA).as_numpy()
...
def test_no_backend_property_access_fails():
    ...
    sad_img = CaikitImage.__new__(CaikitImage)
...
def test_default_format_pil():
    ...
    assert CaikitImage.from_proto(proto_img)._backend._export_format == default_format

### Tests for other properties
def test_dm_props():
    """Test that our row, col, and channel properties return the correct values."""
    im = CaikitImage(image_data=PIL_IMG_DATA)
    assert im.rows == 100
